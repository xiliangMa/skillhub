...
func SeedData() error {
	...
	if cfg.GitHub.Token != "" {
		log.Println("GitHub token found, attempting to sync real skills...")

		// 运行GitHub同步
		if err := crawler.RunScheduledTask("sync_github_skills"); err != nil {
			log.Printf("GitHub sync failed: %v", err)
			log.Println("Falling back to mock data...")
			return createMockData(db)
		}

		// 检查同步后是否有技能
		db.Model(&models.Skill{}).Count(&count)
		if count > 0 {
			log.Printf("Successfully synced %d real skills from GitHub", count)
		...
		log.Println("GitHub sync completed but no skills found, falling back to mock data...")
		return createMockData(db)
	}

	log.Println("No GitHub token configured, creating mock data...")
	return createMockData(db)
}
